...
This is done by editing the `mycreds.json` file which lives in the root of the adsales-api app.
...
Note that only one user (Registrar) is added to the `users` section because adsales-api allows you to create new users. 
...
Once you have edited `mycreds.json` you are ready to run adsales-api. 

1. Continue where you left off in [adsales-api](../README.md).
